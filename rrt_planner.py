...
# Max attempts to find a path - increased for complex obstacle avoidance
DEFAULT_MAX_ITERATIONS = 20000
# Probability (0-1) of sampling the goal state directly - increased for faster convergence
DEFAULT_GOAL_BIAS = 0.2
# How close in distance (meters) to the goal is acceptable - increased for faster success
DEFAULT_GOAL_DIST_TOLERANCE = 25.0
# How close in angle (radians) is acceptable - increased for faster success
DEFAULT_GOAL_ANGLE_TOLERANCE = math.radians(15.0)
...
class RRTNode:
    ...
    def __init__(self, x: float, y: float, heading: float, parent_index: Optional[int] = None,
        ...
        # Typically path length from the root node
        self.cost_from_start = cost_from_start
        # Geometry of the path segment from parent to this node
        self.path_segment_geom = path_segment_geom
...
def get_dubins_path_segment(from_node: RRTNode, to_state: Tuple[float, float, float],
    ...
    if not all(map(math.isfinite, start_pose + end_pose_target)):
        print(
            f"WARN: Invalid start or end pose for Dubins: {start_pose}, {end_pose_target}")
    ...
    try:
        ...
        if turn_radius <= 1e-6:
            ...
            dist_to_target = calculate_distance_euclidean(
                start_pt, end_pt_target)
            ...
            if dist_to_target > 1e-6:
                interp_pt = start_pt + \
                    (end_pt_target - start_pt) * (actual_dist / dist_to_target)
            else:
            ...
            new_heading = math.atan2(
                interp_pt.y() - start_pt.y(), interp_pt.x() - start_pt.x())
            new_node = RRTNode(interp_pt.x(), interp_pt.y(), new_heading)
            return new_node, line_geom, actual_dist

        # Proceed with Dubins
        modes, lengths, radii = dubins_path.dubins_path(
            start_pose, end_pose_target, turn_radius)
        ...
        # Smaller step for projection, minimum 0.1
        proj_max_line_dist = max(target_dist / 5.0, 0.1)
        # Minimum 0.1 to avoid very small values
        proj_max_curve_dist = max(target_dist / 5.0, 0.1)

        # Extra protection against division by zero or very small values
        if turn_radius > 1e-3:  # Increased threshold for safety
            proj_max_curve_angle = (
                proj_max_curve_dist * 360) / (2 * math.pi * turn_radius)
        else:
        ...
        projected_points = dubins_path.get_projection(
            start=start_pose, end=end_pose_target, solution=solution)
        ...
        for i, p_data in enumerate(projected_points):
            ...
            # Dubins get_projection seems to return heading in degrees.
            # --- DEBUG ---
            ...
            segment_heading_rad = math.atan2(dy, dx) if (
                abs(dx) > 1e-6 or abs(dy) > 1e-6) else prev_head
            ...
            if accumulated_dist + segment_dist >= target_dist:
                ...
                # Use heading of the segment leading to interpolated point
                final_heading = segment_heading_rad

                path_vertices.append(interp_pt)
                final_node = RRTNode(
                    interp_pt.x(), interp_pt.y(), final_heading)
        ...
        if final_node is None:
            ...
            last_segment_heading_rad = math.atan2(dy, dx) if (
                abs(dx) > 1e-6 or abs(dy) > 1e-6) else prev_head

            final_node = RRTNode(last_pt.x(), last_pt.y(),
                                 last_segment_heading_rad)
            # Actual length is the accumulated dist
            final_segment_length = accumulated_dist
...
def find_rrt_path(start_pose: tuple[float, float, float],
    ...
    if not obstacles:
        ...
        try:
            ...
            # Nearly same heading
            if abs(normalize_angle(start_pose[2] - end_pose[2])) < 0.1:
                print(
                    "Start and end headings are aligned, trying a straight line path...")
            ...
            if projected_points and len(projected_points) > 1:
                ...
                print(
                    "Dubins path calculation succeeded but returned no points. Creating a simple path.")
                ...
                simple_path = QgsGeometry.fromPolylineXY(
    ...
    for iteration in range(max_iterations):
        ...
        if nearest_node_index == -1:
            continue  # Should not happen if tree has start node
        ...
        if is_safe:
            ...
            new_node_candidate.cost_from_start = nearest_node.cost_from_start + \
                segment_len  # Use actual segment len
            tree.append(new_node_candidate)
            new_node_index = len(tree) - 1

            # 6. Goal Check
            final_node = new_node_candidate
            dist_to_goal = calculate_distance_euclidean(
                final_node.point(), end_point)
            ...
            if iteration % 50 == 0 and dist_to_goal < step_size * 3:
                ...
                if goal_node and goal_path_geom and goal_segment_len is not None:
                    ...
                    if direct_to_goal_safe:
                        ...
                        print(
                            f"RRT: Direct connection to goal found at iteration {iteration + 1}")
                        ...
                        # Start with the direct goal connection
                        path_geometries = [goal_path_geom]
                        ...
                        while current_index is not None:
                            ...
                            if node.path_segment_geom and node.parent_index is not None:
                                path_geometries.insert(
                                    0, node.path_segment_geom)
                            ...
                            for i, geom in enumerate(path_geometries):
                                if geom.type() == QgsWkbTypes.LineGeometry:
                                    ...
                                    if i == 0:
                                        ...
                                        # Include all vertices from first segment
                                        all_vertices.extend(
                                            [QgsPointXY(v.x(), v.y()) for v in vertices])
                                    elif len(vertices) > 1:
                                        # Convert QgsPoint to QgsPointXY for subsequent vertices
                                        # Skip first vertex of subsequent segments
                                        all_vertices.extend(
                                            [QgsPointXY(v.x(), v.y()) for v in vertices[1:]])

                        if len(all_vertices) < 2:
                            return None  # Not enough points

                        final_path_geom = QgsGeometry.fromPolylineXY(
                            all_vertices)
            ...
            if dist_to_goal <= goal_tolerance_dist and angle_diff <= goal_tolerance_angle:
                ...
                if not path_geometries:
                    return None  # Should not happen if goal node has parent
                ...
                try:
                    ...
                    if not final_path_geom.isEmpty() and is_valid:
                        ...
                        print(
                            "RRT: Failed to combine path segments into valid geometry.")
